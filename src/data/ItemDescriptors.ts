...
export type IconCallback = (inventory: PlayerInventory) => string;
...
export const itemDescriptors: IDictionary<IItemDescriptor> =
{
    // Special items
    ...
    bow: {
        name: 'bow',
        icon: function (inventory: PlayerInventory)
        {
            if (inventory.silverArrows && inventory.arrows)
            {
                return 'items/bow_and_silver_arrow.png';
            }
            else if (inventory.arrows)
            {
    ...
    lantern: {
        ...
        particle: {
            ...
            repeat: 0,
            spacing: 2,
            hitArea: new Phaser.Geom.Rectangle(0, 8, 8, 8),
        },
    },
};
